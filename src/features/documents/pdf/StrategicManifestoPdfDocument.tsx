...
export type StrategicManifestoPdfData = {
  title?: string;
...
export default function StrategicManifestoPdfDocument({
  data,
}: {
  data: StrategicManifestoPdfData;
}) {
  const safeTitle =
    sanitizePdfText(data.title) || 'Manifiesto Estratégico';
  ...
  const safePositioning = sanitizeList(data.positioning_recommendations);
  const safeMessages = sanitizeList(data.message_recommendations);

  return (
    <Document
      title={safeTitle}
      author="Guionista de Astucia"
      subject="Manifiesto Estratégico"
    ...
    >
      <Page size="A4" style={styles.page} wrap>
        ...
        {safePositioning.length > 0 ? (
          <View style={styles.listBlock}>
            <Text style={styles.listTitle}>Recomendaciones de posicionamiento</Text>
            <View>{renderBulletList(safePositioning)}</View>
          </View>
        ) : null}
        ...
        {safeMessages.length > 0 ? (
          <View style={styles.listBlock}>
            ...
            <View>{renderBulletList(safeMessages)}</View>
          </View>
        ) : null}

        <Text style={styles.footer}>Manifiesto Estratégico</Text>
      </Page>
    </Document>
  );
}