...
import { generateReelsAdvancedDocument } from '../../features/documents/services/documentGeneration';

type Answers = Record<string, string>;

export default function ReelsAdvancedReviewPage() {
  ...
  const sections = advancedMode.steps
    .map((step) => ({
      ...step,
  ...
  async function handleGenerate() {
    ...
    try {
      await generateReelsAdvancedDocument(sessionId);

      navigate(`/documents/reels_scripts/advanced/result?sessionId=${sessionId}`);
    } catch (error: any) {
      console.error('Error al generar guion avanzado:', error);
      ...
      setErrorMessage('No se pudo generar el guion con Gemini.');
    } finally {
        ...
        <div className="mx-auto max-w-5xl">
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl sm:p-6">
            ...
            <h1 className="mt-2 text-2xl font-bold text-white sm:text-3xl">
              Revisa tu brief avanzado antes de generar
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">
              Cuando generes, Gemini convertira este brief en un guion final + caption
              con mayor control estrategico.
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <button
              type="button"
              onClick={() => navigate('/documents/reels_scripts/advanced/wizard')}
            ...
            >
              {isGenerating ? 'Generando...' : 'Generar guion con IA'}
            </button>
          </div>
        </div>
      </main>
