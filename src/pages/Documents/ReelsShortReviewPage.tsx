...
import { generateReelsShortDocument } from '../../features/documents/services/documentGeneration';

type Answers = Record<string, string>;

export default function ReelsShortReviewPage() {
  ...
  const sections = shortMode.steps
    .map((step) => ({
      ...step,
  ...
  async function handleGenerate() {
    ...
    try {
      await generateReelsShortDocument(sessionId);

      navigate(`/documents/reels_scripts/short/result?sessionId=${sessionId}`);
    } catch (error: any) {
      console.error('Error al generar guion corto:', error);
        ...
        <div className="mx-auto max-w-5xl">
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl sm:p-6">
            ...
            <h1 className="mt-2 text-2xl font-bold text-white sm:text-3xl">
              Revisa tu brief antes de generar
            </h1>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <button
              type="button"
              onClick={() => navigate('/documents/reels_scripts/short/wizard')}
