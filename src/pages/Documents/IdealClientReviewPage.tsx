...
import { generateIdealClientDocument } from '../../features/documents/services/documentGeneration';

type Answers = Record<string, string>;

export default function IdealClientReviewPage() {
  ...
  const sections = idealClientManifestoFlow.steps
    .map((step) => ({
      ...step,
  ...
  async function handleGenerate() {
    ...
    try {
      await generateIdealClientDocument(sessionId);

      navigate(
        `/documents/ideal_client_manifesto/result?sessionId=${sessionId}`
      );
    } catch (error) {
      console.error(error);
        ...
        <div className="mx-auto max-w-5xl">
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl sm:p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">
              Cuando generes, Gemini convertira este formulario en tu documento final
              de Cliente Ideal.
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <button
              type="button"
              onClick={() => navigate('/documents/ideal_client_manifesto/wizard')}
