...
import { generateInventoryOfSelfDocument } from '../../features/documents/services/documentGeneration';

type Answers = Record<string, string>;

export default function InventoryOfSelfReviewPage() {
  ...
  const sections = inventoryOfSelfFlow.steps
    .map((step) => ({
      ...step,
  ...
  async function handleGenerate() {
    ...
    try {
      await generateInventoryOfSelfDocument(sessionId);

      navigate(
        `/documents/inventory_of_self/result?sessionId=${sessionId}`
      );
    } catch (error: any) {
      console.error('Error al generar inventario del ser:', error);
      ...
      setErrorMessage('No se pudo generar el documento con Gemini.');
    } finally {
        ...
        <div className="mx-auto max-w-5xl">
          <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl sm:p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">
              Cuando generes, Gemini convertira estas respuestas en el documento
              final del Inventario del Ser.
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <button
              type="button"
              onClick={() => navigate('/documents/inventory_of_self/wizard')}
