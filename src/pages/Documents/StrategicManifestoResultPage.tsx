...
import StrategicManifestoPdfDocument from '../../features/documents/pdf/StrategicManifestoPdfDocument';
import { getDocumentSessionById } from '../../features/documents/services/documentSessions';
import { exportStrategicManifestoDocx } from '../../features/documents/utils/exportStrategicManifesto';
...
export default function StrategicManifestoResultPage() {
    ...
    async function loadDocument() {
      ...
      try {
        ...
        const { data: savedScripts, error: scriptLookupError } = await supabase
          ...
          .select('id')
          .eq('module', 'strategic_manifesto')
          .eq('script_type', 'strategic_manifesto_document')
        ...
        console.error(error);
        setErrorMessage('No se pudo cargar el manifiesto generado.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  async function handleExportDocx() {
    try {
      ...
      await exportStrategicManifestoDocx(generatedContent);
  ...
  const pdfFileName = `${sanitizeFileName(
    generatedContent.title ?? 'manifiesto-estrategico'
  )}.pdf`;
        ...
        <div className="mx-auto max-w-5xl">
          {isLoading ? (
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 text-center backdrop-blur-xl">
              <p className="text-sm text-white/65">Cargando manifiesto generado...</p>
            </div>
          ) : errorMessage ? (
            ...
            <>
              <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl sm:p-6">
                ...
                <h1 className="mt-2 text-2xl font-bold text-white sm:text-3xl">
                  {generatedContent.title ?? 'Manifiesto Estratégico'}
                </h1>
              ...
              <div className="mt-6 space-y-4">
                ...
                {(generatedContent.positioning_recommendations ?? []).length > 0 ? (
                  <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                    <h2 className="text-lg font-semibold text-white">
                      Recomendaciones de posicionamiento
                    </h2>
                    <ul className="mt-4 space-y-3 text-sm leading-7 text-white/60 sm:text-base">
                      {generatedContent.positioning_recommendations?.map((item) => (
                        <li key={item}>• {item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {(generatedContent.message_recommendations ?? []).length > 0 ? (
                  <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
                    <h2 className="text-lg font-semibold text-white">
                      Recomendaciones de mensaje
                    </h2>
                    <ul className="mt-4 space-y-3 text-sm leading-7 text-white/60 sm:text-base">
                      {generatedContent.message_recommendations?.map((item) => (
                        <li key={item}>• {item}</li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <PDFDownloadLink
                    document={<StrategicManifestoPdfDocument data={generatedContent} />}
